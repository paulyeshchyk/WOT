...
extension NSManagedObject: JSONMapperProtocol {
    ...
    @objc
    open func mapping(fromJSON jSON: JSON, into context: NSManagedObjectContext, parentPrimaryKey: WOTPrimaryKey, linksCallback: @escaping ([WOTJSONLink]?) -> Void) { fatalError("not implemented")}

    @objc
    open func mapping(fromArray array: [Any], into context: NSManagedObjectContext, parentPrimaryKey: WOTPrimaryKey, linksCallback: @escaping ([WOTJSONLink]?) -> Void) { fatalError("not implemented")}
}
