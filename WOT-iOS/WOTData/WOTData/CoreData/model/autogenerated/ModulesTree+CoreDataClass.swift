...
extension ModulesTree {
    override public func mapping(context: NSManagedObjectContext, fromJSON jSON: JSON, pkCase: PKCase, persistentStore: WOTPersistentStoreProtocol?) throws {
        ...
        // MARK: - CurrentModule
        let currentModuleHelper = ModulesTree.CurrentModuleJSONAdapterHelper(objectID: self.objectID, identifier: nil, persistentStore: persistentStore)
        ...
        persistentStore?.fetchRemote(context: context, byModelClass: Module.self, pkCase: currentModulePK, keypathPrefix: nil, instanceHelper: currentModuleHelper)

        // MARK: - NextModules
        let nextModulesHelper = ModulesTree.NextModulesJSONAdapterHelper(objectID: self.objectID, identifier: nil, persistentStore: persistentStore)
        ...
        nextModules?.forEach {
            ...
            persistentStore?.fetchRemote(context: context, byModelClass: Module.self, pkCase: modulePK, keypathPrefix: nil, instanceHelper: nextModulesHelper)
        }

//        // MARK: - NextTanks
//        let nextTanksHelper = ModulesTree.NextTanksJSONAdapterHelper(objectID: self.objectID, identifier: nil)
