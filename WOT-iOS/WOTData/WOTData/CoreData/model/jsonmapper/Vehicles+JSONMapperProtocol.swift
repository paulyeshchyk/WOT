...
extension Vehicles {
    ...
    func mapping(modulestreeJson json: Any?, inContext context: NSManagedObjectContext, parentPrimaryKey: WOTPrimaryKey, linksCallback: @escaping ([WOTJSONLink]?) -> Void) -> [ModulesTree]? {
        ...
        moduleTreeJSONArray.keys.forEach { (key) in
            ...
            guard let moduleTree = mapping(moduletreeJson: moduleTreeJSON, module_id: module_id, into: context, parentPrimaryKey: parentPrimaryKey, linksCallback: linksCallback) else { return }
    ...
    func mapping(moduletreeJson json: Any?, module_id: NSNumber, into context: NSManagedObjectContext, parentPrimaryKey: WOTPrimaryKey, linksCallback: @escaping ([WOTJSONLink]?) -> Void) -> ModulesTree? {
        ...
        result.mapping(fromJSON: json, into: context, parentPrimaryKey: parentPrimaryKey, linksCallback: linksCallback)
        return result
    }
}
