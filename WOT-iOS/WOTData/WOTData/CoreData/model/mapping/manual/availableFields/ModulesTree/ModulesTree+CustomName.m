...
- (NSString *)customName {
    ...
    if ((type & WOTModuleTypeChassis) == WOTModuleTypeChassis) {
        ...
        result = [[self.nextChassis anyObject] performSelector:@selector(name)];
    }
    if ((type & WOTModuleTypeEngine) == WOTModuleTypeEngine) {
        
        result = [[self.nextEngines anyObject] performSelector:@selector(name)];
    }
    if ((type & WOTModuleTypeRadios) == WOTModuleTypeRadios) {
        
        result = [[self.nextRadios anyObject] performSelector:@selector(name)];
    }
    if ((type & WOTModuleTypeTurrets) == WOTModuleTypeTurrets) {
        
        result = [[self.nextTurrets anyObject] performSelector:@selector(name)];
    }
    if ((type & WOTModuleTypeGuns) == WOTModuleTypeGuns) {
        
        result = [[self.nextGuns anyObject] performSelector:@selector(name)];
    }
//    if ((type & WOTModuleTypeTank) == WOTModuleTypeTank) {
//        
//        result = [[self.nextTanks anyObject] performSelector:@selector(name)];
//    }
