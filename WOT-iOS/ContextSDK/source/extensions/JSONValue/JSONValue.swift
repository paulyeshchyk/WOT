...
extension JSONValue {
    ...
    public init?(any: Any?) {
        ...
        } else if let value = any as? Int { self = .int(value)
        } else if let value = any as? Double { self = .double(value)
        } else if let value = any as? Bool { self = .bool(value)
        } else if let jsonArray = any as? [Any] { self = .array(jsonArray.compactMap { JSONValue(any: $0) })
